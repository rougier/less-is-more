...
_bar_edgecolor = RangeDict(
    { range( 1,13): "#000000",
      range(13,36): "none" } )
_bar_shadows = RangeDict(
    { range( 1,18): True,
      range(18,36): False } )
...
_ax3_title_weight = RangeDict(
    { range( 1,20): "bold",
      range(20,36): "normal" } )
_ax3_title_color = RangeDict(
    { range( 1,23): "#000000",
      range(23,36): "#b2b2b2" } )
_xlabel = RangeDict(
    { range( 1, 9): "Type of Food",
      range( 9,36): "" } )
_ylabel = RangeDict(
    { range( 1, 8): "Number of Calories",
      range( 8,36): "" } )
_ax2_background = RangeDict(
    { range( 1, 3): True,
      range( 3,36): False } )
_ax3_background = RangeDict(
    { range( 1, 4): True,
      range( 4,36): False } )
_legend = RangeDict(
    { range( 1, 6): True,
...
ax1.text(0.05, 0.05, "A remake of www.darkhorseanalytics.com",
         ha="left", va="top", family="Source Sans Pro", weight="light")
ax1.text(0.95, 0.05, "Made with matplotlib 2.0",
         ha="right", va="top", family="Source Sans Pro")
...
# -----------------------------------------------------------------------------
ax1.text(0.05, 0.95, ax1_title, ha="left", va="top",
         family="Source Sans Pro", fontsize=32, weight="light")


# -----------------------------------------------------------------------------
if ax2_background:
    ax2.imshow(mpimg.imread('texture.jpg'))
for linewidth,axis in zip(ax2_spines_linewidth,
...
for color,linewidth,axis in zip(ax3_spines_color,
                                ax3_spines_linewidth,
                                ['top','bottom','left','right']):
...
for tick in ax3.get_yticklines():
    tick.set_color(yticklines_color)

ax3.set_xlabel(xlabel, weight="bold")
ax3.set_ylabel(ylabel, weight="bold")

ax3.set_title(ax3_title, weight=ax3_title_weight,
              size=12, loc='left', color=ax3_title_color)
...
if direct_label:
    for x,y in zip(X,Y):
         ax3.text(x, y-25, "%d" % y, ha="center", va="top", color="white",
                  zorder=20, transform=ax3.transData, fontsize=9)

if shadow:
    gauss = DropShadowFilter(5, alpha=.65, offsets=(3, 1))
    shadow = FilteredArtistList(bars, gauss)
    ax3.add_artist(shadow)
    shadow.set_zorder(bars[0].get_zorder() - 0.1)

if legend:
    ax3.legend(bars, labels, edgecolor="k",
               loc='center left', bbox_to_anchor=(1, .65))
